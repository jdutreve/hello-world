...
class FreelanceClient(object):
    ...
    def _create_request_socket(self):
        ...
        request_socket.connect("inproc://toto")
    ...
    def receive(self):
        return self.request_socket.recv(flags=zmq.NOBLOCK).decode(), self.request_socket.recv(flags=zmq.NOBLOCK).decode()
...
class FreelanceAgent(object):
    ...
    def __init__(self, context, command_frontend):
        ...
        self.backend_socket = context.socket(zmq.ROUTER)  # Socket to talk to external servers
    ...
    def on_request_message(self, now):
        ...
        self.sequence += 1
        correlation_id = str(self.sequence).encode()
        self.requests[correlation_id] = request = Request(correlation_id, msg, now)
        # Least Recently Used active server, i.e. queue head
        self.send_request(request)
        # is_request_sent = True   @TODO

    def send_request(self, request):
        self.backend_socket.send(self.actives[0].address, flags=zmq.NOBLOCK|zmq.SNDMORE)
        self.backend_socket.send(request.msg_id, flags=zmq.NOBLOCK | zmq.SNDMORE)
        self.backend_socket.send(request.msg, flags=zmq.NOBLOCK)
        # p("I: SEND REQUEST   %s, ACTIVE!: %s" % (request, self.actives))

    def on_reply_message(self, now):
        # ex: reply = [b'tcp://192.168.0.22:5555', b'1', b'REQ0']
        server_name = self.backend_socket.recv(flags=zmq.NOBLOCK)
        ...
        if data is HEARTBEAT:
            p("I: RECEIVE PONG   %s" % server_name)
            server.connected = True
        else:
            reply = self.backend_socket.recv(flags=zmq.NOBLOCK)
            if data in self.requests:
                self.send_reply(now, data, reply, server_name)
            else:
                pass
                #p("W: TOO LATE REPLY  %s" % data)
    ...
    def send_reply(self, now, correlation_id, data, reply):
        request = self.requests.pop(correlation_id)
        self.reply_nb += 1
        if reply is None:
            p("W: REQUEST FAILED  %s" % correlation_id)
        else:
            p("I: RECEIVE REPLY  %s" % reply)
        self.request_socket.send(request.msg, flags=zmq.NOBLOCK|zmq.SNDMORE)
        self.request_socket.send(data, flags=zmq.NOBLOCK)
...
def do_agent_task(ctx, command_socket):
    ...
    while True:
        ...
        #                 print("I: RETRYING REQUEST  %s, remaining %d" % (request.correlation_id, request.left_retries))
        ...
        #             else:
        #                 agent.failed_nb += 1
        #                 agent.send_reply(now, request.correlation_id, b"FAILED", None)
...
class Request(object):
    ...
    def __init__(self, correlation_id, msg, now):
